...
живёт часами и обязан пережить crash в любой точке, не потеряв ни денег, ни лота. И читать её лучше
как детектив. Жертва известна заранее: процесс, который умрёт на полушаге. Место преступления —
любой шов между двумя транзакциями. А следователь — повторная доставка события: она приходит
на место, восстанавливает картину по уликам и доводит дело до конца. Поймёте settlement —
остальная архитектура перестанет казаться набором правил.
...
повторной неоплате перевыставить лот, а если и это не помогло — признать продажу
несостоявшейся. Три контекста (`auction`, `billing`, `settlement`), длительность — часы
или дни. А теперь наложите на эту длительность реальность эксплуатации: деплои по нескольку
раз в день, OOM, обрывы сети, рестарты подов. Процесс обязательно умрёт посреди шага —
вопрос не «если», а «где именно и что после этого останется на месте».

Отсюда вопрос главы: как гарантировать, что процесс **всегда доходит до терминала** (`Settled`,
...
«Находят пользователи» — это тикет в саппорт через неделю после деплоя, с которого обычно
и начинается расследование. Дальше — о том, как сделать, чтобы расследование заканчивалось
за секунды и без людей.
...
отдельным тестом. Это картотека будущих мест преступления — составленная до того, как
...
шагов без ветвлений («заказ создан, затем письмо отправлено»). Но у settlement ветвящиеся
компенсации (second chance, потом relist, потом fail), бизнес-лимит (`relistGen` cap) и потребность
...
Прежде чем нырять в код — одна бытовая аналогия, которая дальше будет работать на нас.
Сага Molot устроена не как дирижёр, машущий палочкой над оркестром, а как история болезни
в больнице. Пациента ведёт не конкретный врач — врачи меняются сменами, уходят в отпуск, —
а карта: любой, кто её открыл, видит, что сделано, что назначено и чем закончился прошлый
осмотр, и продолжает лечение ровно с этого места. Всё состояние саги — строка в Postgres;
любой «врач» — redelivery события, воркер, HTTP-ретрай — продолжает процесс по записи,
а не по чьей-то памяти. Держите этот образ: половина решений главы из него выводится.
...
Семь состояний, три терминальных — и обратите внимание: тип закрытый, снаружи пакета новое
состояние не сконструировать. Классификация терминальности (`IsTerminal`, там же) написана
switch-ем с паникой в default: новое состояние нельзя добавить, не решив осознанно,
терминально ли оно (правило 12). Сам агрегат (`settlement.go:24`) несёт всё, что нужно
процессу для решений: текущего должника
...
Три строки guard-а перед присваиванием — это весь переход: не то состояние, нулевой или
чужой счёт — `ErrUnexpectedTransition`. Запомните этот sentinel: дальше выяснится, что он —
рабочая лошадка всего протокола. Полная таблица переходов — ARCHITECTURE §6.3; диаграмма —
§6.2. И важная тонкость: `Relisted` — терминал. Перевыставленный аукцион — **новый** аукцион
со своей сагой при закрытии. Процесс не зацикливается внутри одной саги, а порождает
следующую — и `relistGen` нового лота гарантирует, что цепочка оборвётся после первого
перевыставления.
...
Восемь строк — и в них вся политика компенсаций бизнеса: кому second chance, когда
перевыставить, когда сдаться. Результат решения — тоже доменный тип: `NextStep`, закрытый enum из трёх шагов
...
> 1. *decide* — короткое чтение + чистая доменная функция решения; «состояние уже дальше» приводит к ack (дубликат).
...
>    «уже в целевом состоянии» возвращает sentinel, что означает no-op.
> 3. *commit* — короткая `repo.Update` (FOR UPDATE + version): переход state machine. Конфликт означает ack,
...
Прочитайте этот код по комментариям фаз: decide — вставка и перечитанное состояние, effect —
один вызов фасада, commit — короткий `Update`. И заметьте, чего здесь нет: хендлер нигде
не запоминает, «докуда дошёл», — его память это сама строка саги. Теперь о том, почему фазы
нельзя слить в одну транзакцию.
...
**Нюанс.** Дисциплина ack — самое хрупкое место протокола, потому что ошибка в обе стороны
выглядит безобидно. Ack-нете лишнего — похороните аномалию: расхождение контекстов молча
исчезнет из очереди, и узнает о нём бухгалтерия. Не ack-нете дубликат — устроите вечный цикл
ретраев на штатном повторе. Поэтому правило формальное до занудства: в `nil` превращается
ровно один sentinel — `ErrUnexpectedTransition`; всё остальное идёт своим чередом до dead
letter. Когда будете писать свой хендлер — сверяйтесь не с интуицией, а с типом ошибки.
...
«Уже в целевом состоянии — значит nil»: повторный `ConfirmSettlement` по settled-аукциону,
...
Теперь — обещанный детектив. ARCHITECTURE §6.7 — таблица всех точек падения (crash seams)
с предписанным поведением redelivery: та самая картотека мест преступления, и **на каждый шов
есть тест**. Метод следствия везде один. Процесс умер — осталась сцена: строка саги в каком-то
состоянии, что-то уже совершённое в соседних контекстах. Следователь — повторная доставка
входа — не опрашивает свидетелей и не доверяет ничьей памяти: он читает вещественные
доказательства (фактическое состояние в Postgres), повторяет действия — и идемпотентность
гарантирует, что повтор безопасен. Тесты — лучшая документация протокола; разберём четыре дела
(`internal/settlement/app/handlers_test.go`).

**Шов 1: после INSERT саги, до IssueInvoice.** Сцена: строка саги существует, счёта нет —
то самое «полусостояние», которым нас пугала наивная цепочка вызовов. В наивной системе оно
зависло бы навсегда; тест воспроизводит и убийство, и расследование:
...
Восстановим хронологию. Хендлер вернул ошибку, шина не подтвердила сообщение — и следователь
(redelivery) приходит и проходит тот же путь с самого начала: `Add` — no-op
(`INSERT ... ON CONFLICT (auction_id) DO NOTHING`), `Get` читает улику — сага в `Started`, —
decide заключает «счёт ещё нужен», effect выставляет, commit довершает. Заметьте: сага
не «восстанавливается» особым recovery-кодом — она **продолжается** тем же хендлером,
что и в первый раз. Это и есть idempotent continuation, главный приём всей главы.

**Шов 2: после IssueInvoice, до commit.** Зеркальное дело: эффект совершён, фиксация
потеряна. На сцене — счёт, выставленный в billing, и сага, которая по своим записям всё ещё
в `Started`: мир ушёл вперёд, протокол об этом ещё не знает. Тест
`"seam: crash after IssueInvoice before the commit"` (`handlers_test.go:88`) скриптует ошибку
на `repo.Update`. Redelivery повторяет effect — и вот где окупается идемпотентность фасада:
billing видит знакомый детерминированный id, отвечает «уже выставлен», то есть nil, и хендлер
...
Фасад вызван **дважды**, но счёт в системе один. Двойной вызов — норма протокола;
двойной счёт был бы преступлением.

**Шов 3: между двумя эффектами компенсации (MarkSaleFailed и Relist).** Ветка relist делает
два gateway-вызова подряд — значит, между ними есть шов, и однажды процесс умрёт именно там:
...
Обратите внимание, чего здесь нет: чекпоинтов «докуда дошёл». Следователь не полагается
на черновики погибшего — он повторяет **оба** действия: `MarkSaleFailed` оказывается no-op
(продажа уже провалена), `Relist` выполняется. А одинаковый `newID` в обоих вызовах
(см. 10.8) страхует и следующий уровень: упади и второй прогон, третий не создал бы
лота-двойника.

**Шов 4: version-конфликт на commit — конкурентный дубль.** Последнее дело — не про смерть,
а про двойников: два следователя пришли на одну сцену. Два экземпляра обработчика (или
...
дело уже закрыто коллегой — повторять нечего. Сведём метод следствия к одной фразе: **любой
обрыв лечится повторной доставкой входа** — decide читает фактическое состояние, эффекты
повторяются как no-op, commit довершает переход. Ни одна точка падения не требует ни ручного
вмешательства, ни специального recovery-кода: ночной инцидент превращается в строку
в логах ретраев.

**Совет из практики.** Когда будете строить свою сагу, заведите карту швов раньше, чем
напишете первый хендлер: выпишите каждую пару «эффект и его commit» и каждый промежуток между
двумя эффектами — это и есть ваши будущие места преступления. Скриптуемые фейки фасадов
(как `f.billing.issueErr` в тестах выше) позволяют убить процесс в любом из них одной
строкой. Если для какого-то шва тест не пишется — это не недостаток тестируемости, это дыра
в протоколе: скорее всего, эффект не идемпотентен или decide не переживёт повтора.

### 10.5. Fast-forward: событие как доказательство эффекта

Самое тонкое дело протокола — улика, которая приходит раньше, чем сага узнала о собственном
решении. Сценарий: сага в `AwaitingPayment`, счёт истёк, decide выбрал `StepAwardRunnerUp`,
effect `AwardToRunnerUp` **совершён** — аукцион переназначил победителя и опубликовал
`WinnerReassignedV1` через свой outbox — а commit `AwardingRunnerUp` погиб вместе
с процессом. И вот `WinnerReassignedV1` прилетает в сагу, которая по своим записям всё ещё
в `AwaitingPayment`. Событие из будущего, которого сага «не заказывала».

Наивный следователь отбросил бы такую улику как не вяжущуюся с делом: guard «принимаю
`WinnerReassignedV1` только из `AwardingRunnerUp`» дал бы `ErrUnexpectedTransition`, затем ack —
и событие потеряно навсегда. Дальше тупик: redelivery `InvoiceExpiredV1` со временем дотащит
сагу до `AwardingRunnerUp`, но `WinnerReassignedV1` уже подтверждён шине и не придёт — счёт
attempt-2 не выставится никогда. ARCHITECTURE §6.3 фиксирует это отдельной строкой таблицы
переходов:
...
потому что **само событие — неподделываемое вещественное доказательство совершённого
эффекта**. Аукцион физически не может опубликовать `WinnerReassignedV1` без закоммиченного
переназначения — это outbox из главы 9 гарантирует конструктивно
...
Смотрите, как устроена проверка подлинности улики: мало принять событие из «неположенного»
состояния — нужно сверить отпечатки. Победитель и цена обязаны совпасть с записанным
runner-up-ом; любое расхождение — уже не перемотка, а межконтекстная аномалия. Тест
закрепляет ровно этот сценарий (`internal/settlement/app/handlers_test.go`):
...
И заметьте различение внутри guard-а: совпавший runner-up — перемотка вперёд, несовпавший —
`ErrWinnerMismatch` в dead letter, к оператору. Fast-forward — не «принимаем всё подряд»,
а строго верифицированное доказательство: улика принимается, только если сходится с делом
до последней цифры.
...
`ErrOfferAlreadyPaid`, и ответом будет 409. Один re-read различает «исход совпал» и «исход противоположный»,
...
Вспомните историю болезни: пациент не носит с собой будильник врача — время процедур держит
расписание отделения, а карта лишь фиксирует, что процедура состоялась. Сага — карта,
не будильник.

### 10.8. Детерминированные UUID — идемпотентность по построению

Остался последний слой защиты — тот, на котором держались алиби всех предыдущих дел.
Идемпотентность фасадов должна на чём-то стоять: по какому ключу billing поймёт, что «такой
счёт уже есть»? Molot выводит идентификаторы **детерминированно** из бизнес-ключей
(`internal/settlement/app/command/deterministic_ids.go`):
...
**тот же** id: у счёта attempt-1 одного аукциона ровно одна возможная идентичность,
как и у его relist-двойника (`RelistIDFor`). Заметьте «UNIQUE constraints as the second line»:
...
**Где вы на это наступите.** Случайный ID на пути ретрая — дефект с идеальной маскировкой:
на ревью выглядит невинно (что привычнее `uuid.New()`?), все тесты зелёные, на стейдже
работает месяцами. Стреляет он при совпадении двух условий — ретрай или redelivery плюс уже
совершённый эффект, — то есть именно тогда, когда вокруг и так инцидент. Дубль счёта при
упавшем billing-е находят не тесты, а клиент, получивший два письма об оплате. Привычка-
страховка простая: в коде саг и хендлеров событий считайте любой `uuid.New()` находкой для
ревью — пусть автор докажет, что этот путь никогда не повторяется.
...
нет цикла между settlement и auction, единая модель доставки. Минусы, из-за которых отклонено:
...
Восемь способов сломать сагу — все проверены чужими руками:
...
3. **Ack всего подряд.** «Поймал ошибку — значит ack, чтобы не зациклиться» хоронит аномалии.
...
7. **Швы без тестов.** «Мы везде идемпотентны» — гипотеза, пока на каждый шов карты §6.7
   нет теста «упали здесь — redelivery доводит до правильного терминала». Непротестированный шов
...
Проектируете свою сагу — пройдитесь по списку; каждый пункт здесь — закрытое дело
из этой главы:
...
- [ ] Каждый шаг — три фазы: decide, effect, commit; gateway-вызовы строго вне транзакции саги.
- [ ] Каждый эффект идемпотентен: «уже в целевом состоянии» означает no-op; ключи детерминированные
...
- [ ] Карта crash seams написана; на каждый шов — тест idempotent continuation
      («упали здесь — redelivery доводит до корректного терминала»).
...
- `docs/adr/0004-settlement-saga.md` — решение «оркестрация + sync-фасады + три фазы decide,
  effect, commit», трейдофф против all-async (Д2/Д3), условия пересмотра.
