...
Восстановим хронологию. Хендлер вернул ошибку → шина не подтвердила сообщение → следователь
...
напишете первый хендлер: выпишите каждую пару «эффект → commit» и каждый промежуток между
...
`WinnerReassignedV1` только из `AwardingRunnerUp`» дал бы `ErrUnexpectedTransition` → ack —
