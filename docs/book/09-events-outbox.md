...
**Dual-write.** Наивный async: `repo.Save(auction)` → `broker.Publish(event)`.
...
Но цепочки событий складывают задержки: participant → проекция auction →
сага — это три хопа, и каждый ждёт своего поллинга. Библиотечный дефолт
