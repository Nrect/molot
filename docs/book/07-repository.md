...
### Postgres: загрузить, применить fn, сохранить — под двойной защитой
...
- [ ] `sql.ErrNoRows` превращается в доменную `NotFoundError`, уникальные констрейнты — в доменные sentinel-ы; драйверные ошибки выше repo не протекают.
