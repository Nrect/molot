...
означает замену тела этих трёх функций: `wmsql.NewSubscriber` на `kafka.NewSubscriber`,
`wmsql.TxFromStdSQL(tx)` — на соответствующий Kafka-продьюсер через Watermill forwarder. Хендлеры,
...
(из саги в billing, из саги в auction) происходят вне транзакций, в effect-фазе, то есть не на
...
- [ ] Все команды через фасад идемпотентны («уже в целевом состоянии» означает nil)
