...
Предыдущие пять глав разобрали, *что* лежит внутри слоёв: агрегаты, репозитории, use cases, события. Эта глава отвечает на вопрос, который возникает сразу после: **кто всё это соединяет и как именно?**

Три решения — где объявлять интерфейсы, как работать со временем и где инстанциировать конкретные типы — кажутся деталями реализации. На практике они определяют, можно ли тестировать систему без sleep, появятся ли циклы импортов через три месяца и сломается ли сборка на старте процесса или в три часа ночи на первом запросе. Ни один из трёх вопросов не имеет «само получится» ответа.

---

## Проблема: соединение разрушает архитектуру тихо

Слои разделены на бумаге. Но граница между ними сделана из конкретных типов, и как только один конкретный тип пересекает слой — начинается каскад: появляются лишние импорты, за ними циклы, за ними «временные» решения, которые живут годами.

Три самых распространённых способа убить чистую архитектуру при соединении:

**Интерфейс объявляет реализация.** Пакет `adapters` выкладывает `IPaymentGatewayAdapter` рядом со своей структурой. App-слой вынужден импортировать `adapters`, чтобы использовать этот интерфейс. Стрелка зависимости развернулась — домен теперь смотрит в сторону инфраструктуры. Интерфейс неизбежно растёт: каждый новый метод адаптера добавляется в интерфейс, хочет того потребитель или нет.

**Время берётся напрямую.** `time.Now()` в методе агрегата или use case выглядит безобидно — пока не приходит задача протестировать анти-снайпинг, экспирацию счёта или продление окна торгов. Тест, который зависит от реального времени, либо flakky, либо использует `sleep`, либо вообще не пишется. Весь класс поведения становится непокрытым.

**Сборка размазана.** Конкретные типы инстанциируются по-чуть-чуть везде: в пакете-хендлере, в `init()`, через глобальные переменные. Ошибка конфигурации обнаруживается не при запуске, а при первом запросе, который попадёт в нужный код-путь. DI-фреймворк с контейнером прячет граф зависимостей за рефлексией — ошибка сборки обнаруживается в рантайме, а не при чтении кода.
...
В стандартной библиотеке Go есть образцовый пример. `io.Writer` объявлен в пакете `io` — там, где он *используется*. Не в пакете `os`, который предоставляет `*os.File`. Не в пакете `bytes`, который предоставляет `*bytes.Buffer`. Любая структура с методом `Write([]byte) (int, error)` молча удовлетворяет интерфейсу — без явного объявления, без импорта пакета-владельца интерфейса.

Следствия для прикладного кода:

- Интерфейс остаётся маленьким: потребитель объявляет ровно те методы, которые ему нужны, — не больше.
- Рукописный fake для теста — пара строк. Нет нужды в моковых фреймворках.
- Циклов импортов не существует структурно: зависимость всегда направлена от реализации к потребителю, а не наоборот.
- Неявная реализация означает, что адаптер не знает о существовании интерфейса потребителя — он просто реализует нужные методы.
...
Это не «абстракция ради абстракции». Это граница между кодом, который можно тестировать без внешних зависимостей, и кодом, который нельзя.
...
Конструкторы паникуют на `nil`-зависимости. Неправильная сборка падает на старте процесса с читаемым сообщением — не при первом запросе в продакшн.
...
Один метод — один интерфейс. Адаптер, реализующий `bidderProfiles`, может содержать десятки методов для других нужд — потребителя это не касается. Тест подставляет `newSpyProfiles()` — рукописный spy из пяти строк.
...
Два метода. Только то, что нужно команде `PayInvoice`. Вся семантика — в комментарии к интерфейсу: идемпотентность, коды ошибок, контракт повтора. Это и есть документ на адаптер — без отдельного wiki-раздела.
...
`auctionGateway` — четыре метода. `billingGateway` — один. Реальный фасад `auctionservice.Facade`, который реализует `auctionGateway`, публичный и может раскрывать гораздо больше возможностей. Сага знает только те операции, которые ей нужны.
...
В домене `auction` метод `PlaceBid` агрегата принимает `now time.Time` параметром. Логика анти-снайпинга описана в `antisnipe.go`:
...
Нет `time.Now()`. Нет глобального состояния. Функция чистая — детерминированная для любого входа.
...
В тесте `h.clock.Now()` возвращает фиксированное значение. Весь сценарий анти-снайпинга — включая граничные условия, cap по числу продлений и гонку «ставка против закрытия» — покрыт детерминированными юнит-тестами. Ни одного `sleep` во всём тестовом наборе:
...
Первый тест проверяет, что ставка за три минуты до дедлайна (при окне пять минут) сдвигает `EndsAt` на десять минут вперёд. Второй тест проверяет гонку: воркер пытается закрыть аукцион по старому дедлайну, но ставка уже продлила окно — `ErrBiddingStillOpen` возвращается корректно. Оба теста — мгновенные, без внешних зависимостей.
...
`internal/monolith/app.go` — единственный файл, где конкретные типы встречаются друг с другом. Комментарий пакета явно документирует это:
...
Продакшн-точка входа инициализирует OTel-провайдеры, подключается к Postgres, передаёт их в `newApplication`:
...
Оба входа делегируют `newApplication` — приватной функции, которая содержит весь граф:
...
Весь граф — в одном функции. Читаешь сверху вниз: сначала инфраструктура, потом каждый контекст, в конце — воркеры. Никакой магии.
...
То же самое в `NewEventHandlers` из `settlement/app/handlers.go`:
...
DI-фреймворк оправдан, когда ручная сборка становится болью: сотни сервисов, зависимости с многоуровневыми графами, необходимость переключать реализации флагами окружения. BOOK_AUDIT правило 44 формулирует условие перехода прямо: `NewApplication`/`NewComponentTestApplication` → общий `newApplication`. Пока один файл справляется — фреймворк не нужен.
...
Плюс: минимальные интерфейсы, нет циклов, рукописные fakes проще моковых фреймворков. Минус: если одна и та же реализация нужна десяти потребителям, возникает дублирование интерфейсов. В Molot это решается через фасады контекстов: публичный тип `auctionservice.Facade` реализует несколько мелких consumer-side интерфейсов молча — каждый потребитель видит только свой срез.

**Ручная сборка — за и против.**

Плюс: граф читается как код, ошибки статически проверяемы, нет зависимости от внешней библиотеки. Минус: при росте числа контекстов файл сборки растёт. Противоядие — структура `Deps` на каждый сервис: именованные поля, не позиционные аргументы, добавление нового параметра не сдвигает всё.

**Panic в конструкторах — за и против.**

Плюс: ошибка конфигурации обнаруживается сразу. Минус: паника в библиотечном коде, которую вызывает сторонний код, — плохой тон. Здесь паники в конструкторах `NewXxxHandler` — это нарушение программного контракта (передан `nil`), аналогично `index out of range`. Это осознанный выбор: ошибки сборки не должны быть обрабатываемыми.

---

## Типичные ошибки

**Интерфейс у реализации.** Пакет `postgres` экспортирует `AuctionRepositoryInterface`. App-слой импортирует `postgres` ради интерфейса. Стрелка зависимости перевернулась. Решение: переместить интерфейс в доменный пакет или оставить его приватным в app-пакете.
...
- **BOOK_AUDIT.md, правило 44** — два composition root (`NewApplication`/`NewComponentTestApplication`) → один общий `newApplication`; component-тесты — happy path only, auth через реальный путь.
