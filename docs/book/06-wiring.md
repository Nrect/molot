...
Это не догма «фреймворки — зло». DI-фреймворк оправдан, когда ручная сборка становится болью: сотни сервисов, многоуровневые графы зависимостей, необходимость переключать реализации флагами окружения. BOOK_AUDIT правило 44 формулирует условие перехода прямо: `NewApplication`/`NewComponentTestApplication` → общий `newApplication`. Пока один файл справляется — фреймворк не нужен. Когда перестанет справляться, вы это заметите без посторонней помощи.
