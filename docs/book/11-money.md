...
- **`Paid` даёт `nil` (204), и Refund НЕ вызывается.** Это самая дорогая ветка
...
- **`Expired | Voided` даёт страховочный Refund, потом 409.** Зачем Refund, если
...
сетевая ошибка превращается в 502 (`Unavailable`, клиент **должен** ретраить — повтор
бесплатен благодаря ключу), отказ PSP — в 409 (`Conflict`, ретраить
...
- **Refund как компенсация vs auth/capture.** Charged-but-expired можно было
  закрыть иначе: hold (авторизация) при начале оплаты, затем capture при коммите.
...
4. **Refund на ветке Paid** «для симметрии». Возврат денег за успешно
   оплаченный счёт на каждом потерянном ответе. Ветка `Paid` возвращает 204 и ничего
...
6. **Компенсация без страховочной ветки.** Refund после проигранной гонки —
   ...
   компенсацию (Refund в pre-check на ветке `Expired|Voided`), деньги клиента
   зависают навсегда.
