...
Адаптер гарантирует строгую последовательность внутри одной транзакции: сначала load (`SELECT ... FOR UPDATE`),
затем guard доступа, затем вызов fn и наконец persist возвращённого значения (инкрементируя version). `return a, nil`
...
состоянии» возвращает соответствующий sentinel — no-op для саги, ретрай безопасен конструктивно.
...
отдельным тестом с `sagaRepoSpy`: скриптованный сбой, затем redelivery, затем проверка корректного
...
(client-generated UUID), `ErrAlreadyClosed` маппится в `nil` в воркере закрытия, `UNIQUE(auction_id, attempt)`
...
Molot: тик запускает лёгкий скан кандидатов по partial index, затем команда на каждого. Воркер не
